...
function getMineSweeper() {
  var oCount = parseInt($('oCount').innerHTML, 10);
  var y, x, max, boxslist = [],
    has = [];
...
function mineBoxClick(ele) { //格子点击
  ...
  if (box.mine == 0) { //不是雷
    ...
    $('start').setAttribute('src', resource_path + 'img/sad.gif');
    //移除添加事件
  ...
  switch (bumpBOx.mousekey) {
    case 2:
      if (_c == 'hidden') {
        ...
        $('oCount').innerHTML = parseInt($('oCount').innerHTML, 10) - 1;
      } else if (_c == 'flag') {
...
function chooseDifficulty() { //难度选择
  ...
  for (var i = 0; i < len; i++) {
    arr[i].onclick = function() {
      ...
      $('start').setAttribute('src', resource_path + 'img/happy.gif');
      startGame();
    };
  }
}
...
window.onload = function() {
  ...
  $('start').onclick = function() {
    this.setAttribute('src', resource_path + 'img/happy.gif');
    getDifficulty();
    startGame();
  };
